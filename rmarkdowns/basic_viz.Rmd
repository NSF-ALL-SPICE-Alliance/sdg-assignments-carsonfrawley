...
```{r}
ggplot(data = sdr_data, aes(x = goal_6_score, y = goal_7_score, color = goal_13_score)) +
  geom_point()
```

Interpretation: From this graph, it seems like the countries that have low Goal 7 scores (Affordable and Clean Energy) tend to have middle level Goal 6 scores (Clean Water and Sanitation). The countries that have high goal 7 scores also tend to have high goal 6 scores. It is interesting that a lot of the countries that performed poorly between goals 6 & 7 tend to have higher goal 13 scores than the other countries.
...
In this histogram, it seems that the higher scores for Goal 13 (Climate Action) are dominated by the Sub-Saharan Africa and the LAC regions. The OECD and MENA countries seem to be more spread out, as some performed well, but others did awful.