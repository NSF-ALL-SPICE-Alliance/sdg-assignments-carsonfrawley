...
```{r}
k2 <- kmeans(sdr_data_imputed, centers = 3)
```
...
```{r}
importance_df <- as.data.frame(rf_water$importance)
```
...
ggplot(importance_df_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  ...
  theme_minimal() +
  labs(title = "Most Important Variables in Predicting Clean Water",
       subtitle = "Top 10",
       y = "SDG Indicator",
...
```{r}
pdp::partial(rf_matmort, pred.var = "normalized_score_sdg7_elecac", plot = TRUE)
```
...
```{r}
pd <- pdp::partial(rf_matmort, pred.var = c("normalized_score_sdg7_elecac", "normalized_score_sdg6_sanita"))
# Default PDP
plotPartial(pd)
```
