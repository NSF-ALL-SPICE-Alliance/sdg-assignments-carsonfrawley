...
```{r, warning=FALSE, message=FALSE}
goals_12and13_lr <- ggplot(sdr_data, aes(x = goal_12_score, 
                     y = goal_13_score,
                     label = country)) +
  geom_point() +
  geom_smooth() +
...
ggplotly(goals_12and13_lr)
```

Since SDG goals 12 and 13 have a very high correlation, it must be because of how closely related they are to each other. SDG 12 is Responsible Consumption and Production, which has to do with sustainability with consumption, production, and tourism, as well as limiting chemicals and waste. These goals heavily impact the climate, so it makes sense that it has a positive correlation with goal 13, which is Climate Action. There are definitely some points in the bottom left of the graph that don't follow the trend, and some of these countries are Qatar, Brunei Darussalam, United Arab Emirates, Bahrain, and Australia.
